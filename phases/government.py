...
from util.tnt_util import placeable_units, compute_tracks, add_next_phase
...
def reveal_tech(G, player, tech):
	...
	assert tech in faction.secret_vault, 'Tech {} must be in {} secret vault: {}'.format(tech, player,
	                                                                                     faction.secret_vault)
	...
	G.logger.write(
		'{} reveals {} from their secret vault (handlimit is now {})'.format(player, tech, faction.stats.handlimit))
...
class Governmnet_Phase(GamePhase):
	
	def encode(self, G):
		
		if 'move_to_post' in G.temp:
			return encode_post_gov_actions(G)
	...
	def execute(self, G, player=None, action=None):
		...
		if 'hack' in G.temp:
			...
			if player == G.temp.hack.source:
				raise NotImplementedError # TODO: optionally allow source player to undo playing an intelligence card
			...
			G.logger.write(
				' (value so far: {}/{})'.format(G.temp.factory_upgrade.value, G.players[player].stats.factory_cost),
				player=player)
			...
			G.logger.write('{} upgrades their IND to {} with factory card values of: {}'.format(player,
			...
			if head == 'factory_upgrade':
				...
				if 'wildcard' in card:
					...
					G.logger.write(
						'{} plays {} adding/removing influence in {}{}'.format(player, card.wildcard, nation, extra))
...
def government_post_phase(G, player=None, action=None):
	if len(G.temp.move_to_post):
		...
		for player, faction in G.players.items():
			handsize = len(faction.hand)
			if player in G.temp.move_to_post \
					and not G.temp.move_to_post[player] \
					and handsize <= faction.stats.handlimit:
				del G.temp.move_to_post[player]
	...
	for nation, dipl in G.diplomacy.neutrals.items():
		...
		if nation not in G.diplomacy.influence:
			if dipl.faction is None:
				...
				G.logger.write(
					'{} lost influence over {} (losing POP={} RES={})'.format(dipl.faction, nation, pop, res))
