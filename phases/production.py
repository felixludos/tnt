...
class Production_Phase(GamePhase):
	...
	def execute(self, G, player=None, action=None):
		...
		if action is None:
			...
			active_player = G.game.turn_order[G.temp.active_idx]
			G.logger.write(
				'{} may spend {} production points'.format(active_player,
				                                           G.temp.prod[active_player].production_remaining))
