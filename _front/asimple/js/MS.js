class MS {
	...
	circle({border = 'red', wBorder = 0, className = '', sz = 50, fill = 'yellow', alpha = 1, x = 0, y = 0} = {}) {
		return this.ellipse({
			className: className,
			w: sz,
			h: sz,
	...
	ellipse({border = 'red', wBorder = 0, className = '', w = 50, h = 25, fill = 'yellow', alpha = 1, x = 0, y = 0} = {}) {
		...
		if (wBorder > 0) {
			...
			ell.setAttribute('stroke', border);
			ell.setAttribute('stroke-width', wBorder);
		}
	...
	showChildren(className) {
		if (!(className in this.children)) {
			unitTestMS(className,'not in',this.children);
			this.children[className] = {isHidden: true};
		}
		if (!this.children[className].isHidden) {
			unitTestMS(this.children[className].isHidden,'seems to be false')
			return this;
		}
		for (const ch of [...this.elem.childNodes]) {
			let cl = ch.getAttribute('class');
	...
	onClick(ev) {
		unitTestMS('click',this.id,this.isEnabled,this.clickHandler)
		if (!this.isEnabled) return;

		if (typeof this.clickHandler == 'function') {
			unitTestMS('calling clickhandler')
			this.clickHandler(ev);

		}
	}
	rect({border = 'red', wBorder = 0, className = '', w = 50, h = 25, fill = 'yellow', alpha = 1, x = 0, y = 0} = {}) {
		...
		if (wBorder > 0) {
			...
			r.setAttribute('stroke-width', wBorder);
		}

		unitTestMS('rect nachher',fill)
	...
	roundedRect({border = 'red', wBorder = 0, className = '', w = 150, h = 125, fill = 'darkviolet', rounding = 10, alpha = 1, x = 0, y = 0} = {}) {
		this.rect({
			className: className,
			w: w,
			h: h,
