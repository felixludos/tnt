class ACombat {
	...
	calc_combat_rep(dataTempCombat) {
		let assets = this.assets;
		let c = dataTempCombat;
		//console.log(c);

		//let dlist = dict2list(G.units.rules);
		//console.log(dlist);
		//let columnHeaders = dlist.sort(fieldSorter(['priority'])).map(x => x.key);
	...
	rep_combat(data, divName) {
		let crep = this.calc_combat_rep(data);
		console.log(crep);
		...
		for (const loc of crep.locs) {
			...
			d.style.color = fg;
			ipal += 1; //randomColor();
			d.style.width = crep.dims.w + 'px'; //c.size.w + 'px';
			d.style.height = crep.dims.h + 'px'; //c.size.h + 'px';
			d.style.border = '1px solid ' + getpal(6); // red';
			d.style.margin = '10px';
			d.style.float = 'left';
			d.style.textAlign = 'center';
			dParent.appendChild(d);

			//make small or equal div centered horizontally and top vertically in big div
			let d1 = document.createElement('div');
			d1.style.width = brep.size.w + 'px';
			d1.style.height = brep.size.h - 25 + 'px';
