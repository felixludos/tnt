class ABattle {
	...
	populate(d, gid, bg, fg){
		...
		for (const u of this.b.fire_order) {
			...
			//let ums = addUnit('u' + u.unit._id, gid, type, u.unit.nationality, u.unit.cv, x + usz, y + usz);
			let ms = this.createUnit('u' + u.unit._id, gid, type, u.unit.nationality);
			ms.setPos(x + usz, y + usz).draw();
			this.updateCv(ms, u.unit.cv);
			this.ms[u.unit._id] = ms;
